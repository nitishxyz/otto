...
/**
 * Custom-drawn "otto" wordmark — no webfont involved.
 *
 * Each glyph is a rounded-corner polygon on a shared metric grid, painted
 * twice: once as a short hard offset extrusion and once as the front face.
 * The extrusion takes a shade of the letter's own fill, matching the
 * `np-solid-*` rule used by `NeoButton`.
 */
...
type LetterKey = 'o1' | 't1' | 't2' | 'o2';

const LETTERS: Array<{ key: LetterKey; d: string; evenOdd: boolean }> = [
...
const PAINT = {
	ink: 'rgb(var(--np-ink))',
	blue: 'rgb(var(--np-blue))',
	lime: 'rgb(var(--np-lime))',
	yellow: 'rgb(var(--np-yellow))',
	coral: 'rgb(var(--np-coral))',
	surface: 'rgb(var(--otto-surface))',
	mono: 'currentColor',
} as const;

type SolidTone = keyof typeof PAINT;

/**
 * Extrusion and edge colour per tone, matching the `np-solid-*` rule: a filled
 * accent casts a shade of itself, so the drop reads as that colour rather than
 * as ink. Neutral tones fall back to the shared shadow.
 */
const CAST: Record<SolidTone, string> = {
	blue: 'rgb(var(--np-blue-cast))',
	lime: 'rgb(var(--np-lime-cast))',
	yellow: 'rgb(var(--np-yellow-cast))',
	coral: 'rgb(var(--np-coral-cast))',
	ink: 'rgb(var(--np-shadow))',
	surface: 'rgb(var(--np-shadow))',
	mono: 'currentColor',
};

/**
 * Per-letter palette. The repeated `tt` shares one colour so the pair reads as
 * a unit, and the two `o`s bracket the word with the primary blue and lime.
 */
const POP_FILL: Record<LetterKey, SolidTone> = {
	o1: 'blue',
	t1: 'coral',
	t2: 'coral',
	o2: 'lime',
...
export type NeoOttoLogoTone = SolidTone | 'pop';

export interface NeoOttoLogoProps {
	/** Rendered height in px; width follows the mark's aspect ratio. */
	height?: number;
	/** `pop` colours each letter; any other tone paints the whole word. */
	tone?: NeoOttoLogoTone;
	/** Extrusion offset in grid units (scales with `height`). */
	depth?: number;
	/** Press the face into its shadow on hover. */
	animated?: boolean;
	className?: string;
	/** Accessible label; also used as the SVG title. */
	label?: string;
}

export function NeoOttoLogo({
	height = 48,
	tone = 'pop',
	depth = 3,
	animated = true,
	...
	const viewWidth = WORD_WIDTH + depth + PAD * 2;
	const viewHeight = BASELINE - ASCENDER_TOP + depth + PAD * 2;
	const width = Math.round((height * viewWidth) / viewHeight);
	const toneFor = (key: LetterKey) => (tone === 'pop' ? POP_FILL[key] : tone);
	const fillFor = (key: LetterKey) => PAINT[toneFor(key)];
	const castFor = (key: LetterKey) => CAST[toneFor(key)];

	return (
		<svg
			width={width}
			height={height}
			viewBox={`${-PAD} ${ASCENDER_TOP - PAD} ${viewWidth} ${viewHeight}`}
			className={cn('np-logo', animated && 'np-logo-press', className)}
			style={{ '--np-logo-depth': depth } as CSSProperties}
			role="img"
			aria-label={label}
		>
			<title>{label}</title>

			{/* Unstroked on both layers: a stroke is centred on the outline, so it
			    would grow the extrusion past the face and read as a halo rather
			    than a clean offset. */}
			<g
				className="np-logo-shade"
			...
			>
				{LETTERS.map((letter) => (
					<path
						key={letter.key}
						d={letter.d}
						fill={castFor(letter.key)}
						fillRule={letter.evenOdd ? 'evenodd' : 'nonzero'}
					/>
				))}
			</g>

			<g className="np-logo-face" stroke="none">
				{LETTERS.map((letter) => (
					<path
						key={letter.key}
						d={letter.d}
						fill={fillFor(letter.key)}
						fillRule={letter.evenOdd ? 'evenodd' : 'nonzero'}
					/>
				))}
			</g>
