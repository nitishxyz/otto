...
const description = [
	...
	'Desktop page popups open as separate native windows preserving the request and opener; they are not controllable Otto tabs. Use open explicitly to create a controllable tab. download clicks a link; completion is reported by the host browser (the desktop app uses the OS Downloads folder).',
	'Native pointer/keyboard input and promise-aware evaluation are macOS-only. Hover succeeds only if WebKit confirms CSS :hover; other clients report unsupported. Other clients emulate common keyboard defaults using untrusted events.',
